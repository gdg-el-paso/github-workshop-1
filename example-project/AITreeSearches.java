...
public class AITreeSearches {
	...
	/**
	 * GENERATE MAP SPACE This method reads the test case text file
	 * 
	...
	// alex was here
	// at the github workshop

	/**
	 * SEARCH ALGORITHMS
	 * 
	 * @param int[][] map_space the 2D map we generated This method calls the
	 *                various search algorithms we're tasked with implementing.
	 */
	 ...
	 * 
	 * @param int[][] map_space the 2D map we generated This method implements BFS
	 *                on our tree from our text file.
	 */
	 ...
	 * 
	 * @param int[][] map_space the 2D map we generated This method implements IDS
	 *                on our tree from our text file.
	 */
	 ...
	 * A* SEARCH
	 * 
	 * @param int[][] map_space the 2D map we generated This method implements A*
	 *                Search on our tree from our text file.
	 */
	 ...
	 * 
	 * @param boolean[][] nodes_visited the nodes already explored Sets the visited
	 *                    nodes in 2D Map Space to false
	 */
	...
	/**
	 * MANHATTAN DISTANCE This method calculates and @return the Manhattan Distance.
	 * 
	 ...
	 * @param path_cost the cost of the path thus far This method prints the path &
	 *                  cost from the goal node to the start node
	 */
	// public static Node[] generate_successor_nodes(Node current_node, Node
	// goal_node, boolean[][] nodes_visited, int[][] map_space) {

	// //We can only move UP, DOWN, LEFT, & RIGHT
	// LinkedList<Node> children_nodes_LL = new LinkedList <Node> ();

	// //Move Right
	// if(current_node.y + 1 < map_space[0].length &&
	// map_space[current_node.x][current_node.y + 1] != 0 &&
	// !nodes_visited[current_node.x][current_node.y + 1]){
	// children_nodes_LL.add(new Node(current_node.x, current_node.y + 1,
	// manhattan_distance(current_node.x, current_node.y + 1, goal_node),
	// current_node.accumulated_path_cost+ map_space[current_node.x][current_node.y
	// + 1], current_node));
	// nodes_visited[current_node.x][current_node.y + 1] = true;
	// }
	// //Move Left
	// if(current_node.y - 1 >= 0 && map_space[current_node.x][current_node.y - 1]
	// != 0 && !nodes_visited[current_node.x][current_node.y - 1]){
	// children_nodes_LL.add(new Node(current_node.x, current_node.y - 1,
	// manhattan_distance(current_node.x, current_node.y - 1, goal_node),
	// current_node.accumulated_path_cost + map_space[current_node.x][current_node.y
	// - 1], current_node));
	...
	// }
	// //Move Up
	// if(current_node.x - 1 >= 0 && map_space[current_node.x - 1][current_node.y]
	// != 0 && !nodes_visited[current_node.x - 1][current_node.y]){
	// children_nodes_LL.add(new Node(current_node.x - 1, current_node.y,
	// manhattan_distance(current_node.x - 1, current_node.y, goal_node),
	// current_node.accumulated_path_cost + map_space[current_node.x -
	// 1][current_node.y], current_node));
	// nodes_visited[current_node.x - 1][current_node.y] = true;
	// }
	// //Move Down
	// if(current_node.x + 1 < map_space.length && map_space[current_node.x +
	// 1][current_node.y] != 0 && !nodes_visited[current_node.x +
	// 1][current_node.y]){
	// children_nodes_LL.add(new Node(current_node.x + 1, current_node.y,
	// manhattan_distance(current_node.x + 1, current_node.y, goal_node),
	// current_node.accumulated_path_cost + map_space[current_node.x +
